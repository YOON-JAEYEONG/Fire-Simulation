...
## 최신 상호작용 변경 (2026-09-09)

기존 건물 NPC에 소화기·문·다른 NPC 안내 상호작용을 연결했다. 문 통과 뒤 위치를
재설정하거나 문을 삭제하지 않으며, 안내가 끝나면 두 NPC 모두 기존 대피 AI로 복귀한다.
카메라는 자동 전환하지 않고 기존 OVERVIEW / PLAYER 조작을 유지한다.

- [문 열기 → 도움 → 두 사람 대피 시연 및 검증](INTERACTION_VISUAL_PREVIEW_KO.md)
- [일반 NPC의 소화기 상호작용](NPC_NATURAL_SUPPRESSION_KO.md)
- [팀별 모듈 병합 계약](NPC_TEAM_MERGE_GUIDE_KO.md)
- 저장소 루트의 `Launch-InteractionPreview.ps1` 또는 `Launch-NaturalNpcInteractions.ps1`을 사용한다.
  Unreal Engine 5.7 설치 위치가 다르면 런처의 엔진 경로를 수정하고 프로젝트 C++를 빌드한다.
- 에디터 C++ 빌드와 자동 테스트 15개 성공. 문/안내 시연의 두 NPC가 실제 출구에 도착하는 것까지 확인했다.
  실행 로그·스크린샷·빌드 산출물과 배포 불가 검토용 에셋은 저장소에 포함하지 않는다.

아래는 초기 NPC 계층·분산·모션 작업의 소개다. 최신 상호작용 실행 방법과 한계는 위 문서를 우선한다.
